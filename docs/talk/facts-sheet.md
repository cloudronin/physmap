...
|---|---|---|---|---|---|---|---|---|
| naca_tn1451 | thermal-fluids | x_over_D | unobservable | PHYSMAP_WINS | clear | benchmark_grade | 40 | 45 |
...
| naca_tn1451 | unobservable | 9 of 9 | 19 |
...
**The home baseline behind each count** — derived beside the bank, from the same data. A count shows a failure that deployment created only where the surrogate was accurate at home. No gate decides it; the counts and rates are the evidence, and the reading is written prose. No row is removed.

| vehicle | home error: how obtained | home wrong (held out) | in-sample | deployed wrong | interpretation |
|---|---|---|---|---|---|
| casper_hypersonic_transition | in-sample fit error, with a held-out refit | 6 of 159 (4%) | 2 of 159 (1%) | 8 of 8 (100%) | Held out, the surrogate is wrong on 6 of 159 home rows (4%); deployed on the quiet tunnel, on 8 of 8 (100%). The failure appears on deployment, and its cause, freestream noise, is not a surrogate input. |
| dirker_water | in-sample fit error, with a held-out refit | 0 of 31 (0%) | 0 of 31 (0%) | 11 of 60 (18%) | Held out, the straight-line surrogate is wrong on 0 of 31 low-Richardson home rows; deployed, on 11 of 60 (18%). The failures appear as buoyancy grows, a cause the surrogate's inputs see only partly. |
| naca_tn1451 | held-out home error | 0 of 40 (0%) | — | 9 of 45 (20%) | Gnielinski is wrong on 0 of 40 fully developed home rows and on 9 of 45 entrance rows (20%), all near the inlet. The failures appear in the entrance region, which the correlation does not cover. |
| jin_sco2_buoyancy | held-out home error | 17 of 17 (100%) | — | 26 of 27 (96%) | The constant-property correlation is wrong on 17 of 17 home rows (100%) and 26 of 27 deployed rows (96%); 11 of 17 home rows lie inside its validated range. It is wrong almost everywhere in this near-critical CO2 data, so the counts cannot show that deployment created the failure. |
| velazquez_sco2 | held-out home error | 386 of 393 (98%) | — | 67 of 67 (100%) | The constant-property correlation is wrong on 386 of 393 home rows (98%) and 67 of 67 deployed rows (100%); only 197 of 393 home rows lie inside its validated range. It is wrong almost everywhere, so the counts cannot show that deployment created the failure. |
| marineau_hypersonic_transition | in-sample fit error, with a held-out refit | 5 of 9 (56%) | 0 of 9 (0%) | 6 of 6 (100%) | In-sample the surrogate looks perfect (0 of 9 wrong); held out, it is wrong on 5 of 9 home rows (56%); deployed, on 6 of 6. Nine home rows cannot separate the two. Its role is the control where the cause is a surrogate input: the input-based detectors see it, and PhysMAP adds nothing. |
| forrest | held-out home error | 0 of 1 (0%) | — | 4 of 4 (100%) | One home row, so there is no home baseline; its values are visual triage estimates, not benchmark-grade. |

Exploratory only, decides nothing (a one-sided Fisher exact test of deployment against held-out home error, chosen after the counts were seen): casper_hypersonic_transition p = 2e-10; dirker_water p = 0.007; naca_tn1451 p = 0.002; jin_sco2_buoyancy p = 1; velazquez_sco2 p = 0.33; marineau_hypersonic_transition p = 0.092; forrest p = 0.2.

**Architecture axis** — three model types per flagged vehicle, each trained on the surrogate inputs and first gated on its held-out error; at percentile 99, the wrong predictions only PhysMAP flags. Testable on 2 of 4 vehicles.
...
| naca_tn1451 | 11 of 11 | 11 of 11 | 11 of 11 |
...
Sources: `data/benchmarks/v0_4_1/matrix_full_seven.json`, `data/benchmarks/v0_4_1/home_baseline.json`, `data/benchmarks/v0_4_1/architecture_axis.json`, `src/physmap/benchmarks/registry.py`.
...
- Applicability, against the measurement at the numerical-error threshold (17.5 %): Gnielinski is within it on 40 of 40 fully developed points and exceeds it on 9 of 45 entrance points, all at x/D ≤ 5, the largest 38 % at x/D 1. PhysMAP places 45 of 45 entrance predictions outside the closure's supported region; the other 36 are numerically acceptable but not physically supported by it.
- The benchmark's NACA cell uses its own split — 40 training rows, 45 test rows — with the same pattern.
